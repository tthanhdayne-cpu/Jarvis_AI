...
from PyQt6.QtCore import (
    QEasingCurve, QMimeData, QObject, QPointF, QRectF, QSize, Qt, QSignalBlocker,
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        sc_mute.activated.connect(lambda: self._toggle_mute("keyboard"))
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._mute_btn.clicked.connect(
            lambda _checked=False: self._toggle_mute("user_click")
        )
    ...
    def _toggle_mute(self, source="user_click"):
        print(f"[JARVIS MIC] toggle_source={source}")
        self._set_microphone_muted(not MICROPHONE_STATE.muted)

    def _set_microphone_muted(self, muted: bool):
        MICROPHONE_STATE.set_enabled(not muted)
        muted = MICROPHONE_STATE.muted
        blocker = QSignalBlocker(self._mute_btn)
        ...
        if self.on_mute_changed:
            self.on_mute_changed(not muted)
...
class JarvisUI:
    def __init__(self, face_path: str, size=None):
        ...
        # ===== SYSTEM TRAY =====
        self.tray = QSystemTrayIcon(self._win)

        icon_path = BASE_DIR / "jarvis.ico"
        self.tray.setIcon(QIcon(str(icon_path)))
        self.tray.setToolTip("JARVIS AI")

        menu = QMenu()

        show_action = QAction("Mở Jarvis", self._win)
        hide_action = QAction("Ẩn Jarvis", self._win)
        quit_action = QAction("Thoát", self._win)
    ...
    def muted(self, v: bool):
        if bool(v) != MICROPHONE_STATE.muted:
            self._win._set_microphone_muted(bool(v))
